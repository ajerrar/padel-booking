...
export class TerrainCardService {
  ...
  // Données "en dur" classées par clubId (ou siteId)
  ...
  // ✅ Méthode propre : tu passes l'id, il te renvoie la liste
  getCourtsByClubId(clubId: number): CourtListModel[] {
    return this.courtsByClubId[clubId] ?? [];
  }

  // Compatibilité : ancienne méthode attendue par le code existant
  getterrains(): CourtListModel[] {
    // retourne tous les courts aplatis de tous les clubs
